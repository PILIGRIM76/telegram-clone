...
class SQLiteStorage {
    ...
    async подключиться() {
        ...
        await this.db.run(`CREATE TABLE IF NOT EXISTS пользователи (
            uid TEXT PRIMARY KEY, публичныйКлюч TEXT, приватныйКлюч TEXT,
            имяПользователя TEXT, аватар TEXT, отпечатокКлюча TEXT, магазин TEXT,
            createdAt TEXT, updatedAt TEXT
        )`);
        await this.db.run(`CREATE TABLE IF NOT EXISTS группы (
            id TEXT PRIMARY KEY, название TEXT, участники TEXT, idВладельца TEXT,
            тип TEXT, токенПриглашения TEXT, createdAt TEXT, updatedAt TEXT
        )`);
        await this.db.run(`CREATE TABLE IF NOT EXISTS сообщения (
            id TEXT PRIMARY KEY, idОтправителя TEXT, текст TEXT, временнаяМетка TEXT,
            статус TEXT, idГруппы TEXT, тип TEXT, media TEXT, mediaType TEXT,
            времяИсчезновения INTEGER, таймерУстановленВ INTEGER, payload TEXT, createdAt TEXT
        )`);
        await this.db.run(`CREATE TABLE IF NOT EXISTS офлайн_сообщения (
            id INTEGER PRIMARY KEY AUTOINCREMENT, uid TEXT NOT NULL, сообщения TEXT,
            createdAt TEXT, updatedAt TEXT, UNIQUE(uid)
        )`);
    ...
    async создатьПользователя(user) {
        ...
        await this.db.run('INSERT INTO пользователи (uid, публичныйКлюч, приватныйКлюч, имяПользователя, аватар, отпечатокКлюча, магазин, createdAt, updatedAt) VALUES (?,?,?,?,?,?,?,datetime("now"),datetime("now"))',
            [uid, user.публичныйКлюч, user.приватныйКлюч, user.имяПользователя, user.аватар, user.отпечатокКлюча, user.магазин]);
        return { uid };
    }
    ...
    async обновитьПользователя(uid, данные) {
        ...
        await this.db.run('UPDATE пользователи SET ' + fields + ', updatedAt = datetime("now") WHERE uid = ?',
            [...Object.values(данные), uid]);
    ...
    async создатьГруппу(group) {
        ...
        await this.db.run('INSERT INTO группы (id, название, участники, idВладельца, тип, токенПриглашения, createdAt, updatedAt) VALUES (?,?,?,?,?,?,datetime("now"),datetime("now"))',
            [id, group.название, JSON.stringify(group.участники || []), group.idВладельца, group.тип, group.токенПриглашения]);
        return { id };
    }
    ...
    async удалитьГруппу(id) {
        await this.подключиться();
        const r = await this.db.run('DELETE FROM группы WHERE id = ?', [id]);
        return r.changes > 0;
    }
    ...
    async сохранитьСообщение(msg) {
        ...
        await this.db.run('INSERT INTO сообщения (id, idОтправителя, текст, временнаяМетка, статус, idГруппы, тип, createdAt) VALUES (?,?,?,?,?,?,?,datetime("now"))',
            [id, msg.idОтправителя, msg.текст, msg.временнаяМетка, msg.статус, msg.idГруппы, msg.тип]);
        return { id };
    }
    ...
    async добавитьОфлайнСообщение(uid, msg) {
        ...
        if (existing) {
            ...
            msgs.push(msg);
            await this.db.run('UPDATE офлайн_сообщения SET сообщения = ? WHERE uid = ?', [JSON.stringify(msgs), uid]);
        } else {
