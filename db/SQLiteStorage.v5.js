...
class SQLiteStorage {
    ...
    async получитьВсеГруппы() {
        await this.подключиться();
        return await this.db.all('SELECT * FROM группы', []);
    }
    ...
    async добавитьУчастникаВГруппу(uid, groupId) {
        ...
        const participants = group.участники ? JSON.parse(group.участники) : [];
        if (participants.includes(uid)) return false;
        participants.push(uid);
        await this.db.run('UPDATE группы SET участники = ? WHERE id = ?', [JSON.stringify(participants), groupId]);
        return true;
    }
